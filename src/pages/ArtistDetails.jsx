...
const ArtistDetails = () => {
  ...
  if (g_artistIdSuccess) {
    console.log("g_ArtistId", g_artistId);
  }
  const {
    data: g_artistData,
  ...
  if (g_artistDataSuccess) console.log("artistData", g_artistData);

  const {
    data: g_ArtistTopSongs,
    isFetching: g_fetchingTopSongs,
    error: g_errorTopSongs,
  } = useGetGeniusArtistTopSongsQuery(g_artistId);
  const [isReadShow, setIsReadShown] = useState(false);
  const toggleBtn = () => {
    setIsReadShown((prevState) => !prevState);
  };
  const { activeSong, isPlaying } = useSelector((state) => state.player);
  const songGrid = g_ArtistTopSongs?.songs?.map((song, i) => {
    return (
      <SongBar
        key={song.id}
        song={song}
  ...
  if (g_isFetching || g_isFetchingArtistData) return <Loader />;
  if (g_error || g_errorArtistData) return <Error />;
    ...
    <div className="flex flex-col w-full text-white h-full">
      ...
      <div className="container flex flex-col px-[200px]  font-poppins  text-white">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 grid-rows-3 gap-2">{songGrid}</div>
        </div>
        <div className="mb-9">
          <div className="text-[50px] font-[600] tracking-wide  mb-[10px] leading-[120px] z-20 ">
            Popular Albums
